	...
	*/
	public function init() {
		...
		if (is_admin()) {
            ...
            add_filter('wp_nav_menu_args', function($args) { if (empty($args["walker"])) $args["walker"] = new WarpMenuWalker(); return $args; });

            // filter widgets that should not be displayed
			add_filter('widget_display_callback', function($instance, $widget, $args) { $warp = Warp::getInstance(); return $warp["widgets"]->get($widget->id)->display ? $instance : false; }, 10, 3);
	...
	*/
	public function _wp() {
		...
		if (($this['config']->get('ie6page') && $this['browser']->isIE6()) || ($this['config']->get('outdated_browser') && $this['browser']->outdatedBrowser())) {
			$this['event']->bind('render.layouts:template', function(&$layout, &$args) {
				$args["title"] = __("Please update to a modern browser", "warp");
				$args["error"] = "browser";
				$args["message"] = __("outdatedBrowser_page_message", "warp");
